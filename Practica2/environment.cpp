...
Environment::Environment(ifstream &infile)
{
	...
	int aldeano = 0;//0;
	int princesa = 1;//random()%2;
	int princeso = 1;//random()%2;
	int leonardo = 1;//random()%2;
	int bruja = 1;//random()%2;
	int profe = 1;//random()%2;
...
void Environment::AcceptAction(Agent::ActionType action)
{
	...
	if (golpeado_){
		...
		if (avanza){
			if (delante[1]=='K'){
				mensaje_+="PK fila: "+to_string(agentPosX_)+" columna: " + to_string(agentPosY_)+". ";
			}
...
void Environment::SenSorStatus(char *terreno, char *objetos, string &mensaje, bool &reiniciar, char &en_uso, char *mochila,
	...
	string valor = to_string(puntuacion_);
	strcpy(puntuacion, valor.c_str());
	finjuego=finJuego_;
	salud = vida+48;
}
...
int Environment::Puntuar(char objeto, char personaje, string &mensaje){
	...
	mensaje+="Te han dado "+to_string(valor)+" puntos";
	puntuacion_+=valor;
	return valor;
}
